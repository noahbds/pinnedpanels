...
local function EnsureFocusValid()
...
function PinnedPanels.SetNavPopup(frame)
    if IsValid(frame) then
        ...
        IM.NavFocusIndex   = TopMostIndex(PinnedPanels.GetNavElements())
        IM.SelectedIndex   = nil
        ResetNavKeyState()
    ...
    if enterNow or rightNow then
        ...
        if it and it.enabled ~= false then
            if it.isSubMenu and it.subItems and #it.subItems > 0 then
                ...
                local ry = cy + 6 + (pnl.SelIndex - 1) * 22
...
local BINDS = {
    ...
    { id = "auto_arrange",     name = "Auto-Arrange Panels",            default = KEY_NONE,     action = function() if PinnedPanels.AutoArrange then
            PinnedPanels.AutoArrange() end end },
    {
        id = "autosize_focused",
    ...
    { id = "reopen_closed", name = "Reopen Last Closed", default = KEY_NONE, action = function() if PinnedPanels.ReopenLastClosed then
            PinnedPanels.ReopenLastClosed() end end },
}
...
local function AdjustValue(el, key)
    ...
    if tgt then
        if tcls:find("ColorCube") then
            ...
            if key == KEY_LEFT then
                s = math.Clamp(s - 0.04, 0, 1)
            elseif key == KEY_RIGHT then
                s = math.Clamp(s + 0.04, 0, 1)
            elseif key == KEY_UP then
                v = math.Clamp(v + 0.04, 0, 1)
    ...
    if cls:find("Slider") then
        ...
        local newVal = math.Clamp(cur + (mx - mn) / 30 * dir, mn, mx)
        el:SetValue(newVal)
    ...
    elseif cls == "DComboBox" and isfunction(el.ChooseOptionID) and el.Choices then
        local n = #el.Choices
        if n > 0 then
...
local function ActivateElement(el)
    ...
    if AdjustKind(el) then
        return
    elseif cls == "DTextEntry" or cls == "DNumberWang" or cls == "TextEntry" then
        local pin = FocusedPin()
        if pin and IsValid(pin.frame) then
    ...
    for i, el in ipairs(elements) do
        if el ~= focusEl and IsValid(el) then
            ...
            if valid then
                ...
                local score = horiz and (math.abs(dx) + math.abs(dy) * 4) or (math.abs(dy) + math.abs(dx) * 4)
        ...
        elseif key == KEY_ENTER then
            if down and not wasDown then
                ...
                if shiftHeld and focusedEl then
                    OpenElementContextMenu(focusedEl)
                elseif focusedEl then
                    ActivateElement(focusedEl)
                end
            end
        elseif down then
            ...
            if not wasDown or ShouldRepeat(key) then
                if IM.SelectedIndex and IsValid(selectedEl) then
                    ...
                    if consume then AdjustValue(selectedEl, key) else IM.SelectedIndex = nil end
                else
...
hook.Add("Think", "PinnedPanels_KeyNav", function()
    ...
    if not anyNavDown then
        for _, b in ipairs(BINDS) do
            if b.key and b.key ~= KEY_NONE and input.IsKeyDown(b.key) then
                anyNavDown = true
                break
            end
        end
